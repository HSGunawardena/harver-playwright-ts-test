...
import { StatusCodeSubPage } from '../pages/status-code-sub-page';
...
test.describe('Status Code Verification', () => {
  ...
  test('verify default page content', async () => {
    await expect(statusCodePage.getPageText).toContainText(statusCodePage.pageText);
    await expect(statusCodePage.getHereLink).toBeVisible();
    await expect(statusCodePage.getPageText).toContainText(statusCodePage.pageSubText);
  ...
  test('verify here link takes user to iana page', async ({ page }) => {
    ...
    await expect.soft(page).toHaveURL('http://www.iana.org/assignments/http-status-codes/http-status-codes.xhtml');
  });

  test('verify 200 status links takes user to correct status code page', async () => {
    await statusCodePage.get200Link.click();
    await expect.soft(statusCodeSubPage.getHeaderTitle).toBeVisible();
    await expect.soft(statusCodeSubPage.getPageText).toContainText('This page returned a 200 status code.');
    await statusCodeSubPage.getHereLink.click();
    await expect.soft(statusCodePage.getPageText).toContainText(statusCodePage.pageText);
  });

  test('verify 301 status links takes user to correct status code page', async () => {
    await statusCodePage.get301Link.click();
    await expect.soft(statusCodeSubPage.getHeaderTitle).toBeVisible();
    await expect.soft(statusCodeSubPage.getPageText).toContainText('This page returned a 301 status code.');
    await statusCodeSubPage.getHereLink.click();
    await expect.soft(statusCodePage.getPageText).toContainText(statusCodePage.pageText);
  });
  ...
  test('verify 404 status links takes user to correct status code page', async () => {
    ...
    await expect.soft(statusCodeSubPage.getHeaderTitle).toBeVisible();
    await expect.soft(statusCodeSubPage.getPageText).toContainText('This page returned a 404 status code.');
    await statusCodeSubPage.getHereLink.click();
    await expect.soft(statusCodePage.getPageText).toContainText(statusCodePage.pageText);
  });
  ...
  test('verify 500 status links takes user to correct status code page', async () => {
    ...
    await expect.soft(statusCodeSubPage.getHeaderTitle).toBeVisible();
    await expect.soft(statusCodeSubPage.getPageText).toContainText('This page returned a 500 status code.');
    await statusCodeSubPage.getHereLink.click();
    await expect.soft(statusCodePage.getPageText).toContainText(statusCodePage.pageText);
  });
});